...
```
```{r}
hold = exp(-1.087e+01+ -6.468e-01 + 5.737e-03*2000 + 3.033e-06  * )
hold/(1+hold)
```
...
for(name in list(james, kim, sam, kennedy)){
  lo = predict(dmod2, newdata=name, price = response)
  newframe[i,2] = (exp(lo)/(1+exp(lo)))*100
  i=i+1
}
...
baseline_returns <- "^GSPC" %>% tq_get(get = "stock.prices",
                                       from = "2018-01-01",
                                       to = "2022-12-31") %>%
  tq_transmute(select= adjusted,
               mutate_fun=periodReturn,
               period= "monthly",
               col_rename = "BaselineReturns")



single_portfolio<-left_join(portfolio_returns,
                            baseline_returns,
                            by = "date")

single_portfolio %>% tq_performance(Ra = StockReturns, Rb = BaselineReturns, performance_fun=table.CAPM) %>%
