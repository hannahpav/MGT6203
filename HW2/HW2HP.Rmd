...
print(paste('tsla min return:', ticker_tsla  %>% drop_na()  %>% slice_min(SimpleReturn)))
...
'tsla'= round(StdDev(stock_returns_monthly_tsla$Ra)*100,2),
'xom'=round(StdDev(stock_returns_monthly_xom$Ra)*100,2),
'appl'= round(StdDev(stock_returns_monthly_aapl$Ra)*100,2),
'gm'= round(StdDev(stock_returns_monthly_gm$Ra)*100,2))
...
mean_table <- data.frame(stock = c('tsla','xom', 'appl', 'gm'), average = c(mean(stock_returns_monthly_tsla$Ra), 
                           mean(stock_returns_monthly_xom$Ra), mean(stock_returns_monthly_aapl$Ra), 
                           mean(stock_returns_monthly_gm$Ra)))
...
cumsumframe[1,] =  c('tsla', round(Return.cumulative(stock_returns_monthly_tsla$Ra, 
                                                     geometric = TRUE) *100,2))
cumsumframe[2,] =  c('xom', round(Return.cumulative(stock_returns_monthly_xom$Ra, 
                                                     geometric = TRUE )*100,2))
cumsumframe[3,] =  c('aapl', round(Return.cumulative(stock_returns_monthly_aapl$Ra, 
                                                     geometric = TRUE )*100,2))
cumsumframe[4,] =  c('gm',   round(Return.cumulative(stock_returns_monthly_gm$Ra, 
                                                     geometric = TRUE )*100,2))
...
## TSLA
tsla_mod = lm(stock_returns_monthly_tsla$Ra ~ ticker_gspc$return_market)
summary(tsla_mod)

## XOM

xom_mod = lm(stock_returns_monthly_xom$Ra ~ ticker_gspc$return_market)
summary(xom_mod)

## AAPL

aapl_mod = lm(stock_returns_monthly_aapl$Ra ~ ticker_gspc$return_market)
summary(aapl_mod)

## GM

gm_mod = lm(stock_returns_monthly_gm$Ra ~ ticker_gspc$return_market)
summary(gm_mod)
...
all_returns$Date = 
all_returns_time = xts(all_returns, order.by = all_returns$Date)
```

```{r}
library(ggplot2)

chart.CumReturns(all_returns_time$Ra.tsla, wealth.index=FALSE, geometric=TRUE)
  #                , plot.engine="ggplot2") + theme_classic() +
