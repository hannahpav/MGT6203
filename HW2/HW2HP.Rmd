...
print(c('max return:', ticker_tsla0 %>% arrange(Date)  %>% slice_max(Ra) %>% select(Date)))
print(c('min return:', ticker_tsla0 %>% arrange(Date)  %>% slice_min(Ra) %>% select(Date)))
```

```{r}

    
print(c('max return:', ticker_xom0 %>% drop_na() %>% arrange(Date)  %>% slice_max(Ra)))
print(c('min return:', ticker_xom0 %>% drop_na() %>% arrange(Date)  %>% slice_min(Ra) %>% select(Date)))
```
...
```{r}
  ...
  join1 = right_join(ticker_tsla0, ticker_xom0, by = 'Date', suffix = c(".TSLA", ".XOM"))
  join2 = right_join(ticker_aapl0, ticker_gm0, by = 'Date', suffix = c(".AAPL", ".GM"))
