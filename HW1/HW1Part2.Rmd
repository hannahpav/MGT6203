...
```
the value of the intercept tells us that when sexmale=0, or when the sex is female, log odds of survival for a female is 0.9818.
...
This means the odds of survival for females are 2.669 times higher than the odds of not surviving.
...
```{r}
## find log odds
lom = Tmodel$coefficients[[1]] + Tmodel$coefficients[[2]]
## find probability
prob = 1/(1+exp(-lom))
round(prob,3)
```

The probability of survival for a male passenger is 0.191

#3D. Logistic Regression Indicator Variables (7 points):
Create indicator variables for the variable, **`passengerclass`**.
Treat **`passengerclass`**= "3rd" as the base case.
Therefore, `first_class` = 1 if `passengerclass` is "1st" or else 0.
`second_class` = 1 if `passengerclass` is "2nd" or else 0.
Create a logistic regression model with the variable, survived, as the response variable and the indicator variables created for passengerclass as the independent model variables.
Using this model, calculate the probability of survival for the passengers in all 3 classes.
Note: Round to three decimals. Example, XX.xxx </ul>

```{r}
## SET DUMMY VARIABLES WITH "3rd class" AT BASE for passengerclass
TitanicSurvival2<- TitanicSurvival %>%
  mutate(pclass1st = ifelse(passengerclass=="1st",1,0))  %>%
  mutate(pclass2nd = ifelse(passengerclass=="2nd",1,0))

## REMOVE 'passengerclass' VARIABLE
TitanicSurvival2 = TitanicSurvival2[,-4]

## CREATE LOGISTIC MODEL
Tmodel2 = glm(survived ~ pclass1st+pclass2nd, data=TitanicSurvival2, family = 'binomial')
summary(Tmodel2)

```
```{r}
## Psurvival(1st)
lo1st = Tmodel2$coefficients[[1]] + Tmodel2$coefficients[[2]]
prob1st = round(1/(1+exp(-lo1st)),3)

## Psurvival(2nd)
lo2nd = Tmodel2$coefficients[[1]] + Tmodel2$coefficients[[3]]
prob2nd = round(1/(1+exp(-lo2nd)),3)

## Psurvival(3rd)
lo3rd = Tmodel2$coefficients[[1]]
prob3rd = round(1/(1+exp(-lo3rd)),3)

paste0('survival 1st class = ', prob1st, '; survival 2nd class = ', prob2nd, '; survival 3rd class = ', prob3rd)
